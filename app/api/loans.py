...
def request_loan(
    ...
    if data.loan_type == "NOVI+":
        # NOVI+: Instant loan for employees, up to 20% of salary, 1% per week, 4 weeks
        # For now, assume amount is valid, TODO: add salary check
        interest_rate = 0.01  # 1% per week
        repayment_weeks = 4
        due_date = datetime.utcnow() + timedelta(weeks=repayment_weeks)
        total_to_repay = data.amount * (1 + interest_rate * repayment_weeks)
        # No guarantors for NOVI+
        guarantors_required = 0
    elif data.loan_type == "ALOBA":
        # ALOBA: Social loan with solidary guarantee
    ...
    if data.loan_type == "ALOBA" and len(data.guarantors) < guarantors_required:
        raise HTTPException(status_code=400, detail=f"At least {guarantors_required} guarantor is required for {data.loan_type}")
    ...
    # For ALOBA, create guarantees
    if data.loan_type == "ALOBA":
